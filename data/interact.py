...
class INTERACTDataset(data.Dataset):
    def __init__(self, config, split_name, data_aug=True, exp_type='HHC'):
    ...
    def _load_summary(self, split_name):
        ...
        import pandas as pd
        unified_frames = np.asarray(pd.read_csv(_summary_file))
    ...
    def _load_all(self):
        ...
        with h5py.File('/scratch/msy9an/data/tro_video/skeleton_{}.h5'.format(self.split), 'r') as file:
            ...
            def print_hierarchy(name, obj):
                if isinstance(obj, h5py.Dataset):
                    ...
                    with h5py.File('/scratch/msy9an/data/tro_video/exo_1_{}.h5'.format(self.split), 'r') as video_file:
                        try:
                            video_name =  (self.unified_frames['/{}'.format(name)])
                            video_data = video_file["{}/{}".format(video_name[1:],"frames")]
        ...
        print (zero_count)
        # all_interact_motion_poses = np.array(all_interact_motion_poses).reshape(-1, all_interact_motion_poses[0].shape[1])
    ...
    def _load_all_video_chunks(self):
        ...
        with h5py.File('/scratch/msy9an/data/tro_video/exo_1_{}.h5'.format(self.split), 'r') as file:
            # Print the keys (group names) at the top level
            # Iterate through the keys and print the hierarchy
            def print_hierarchy(name, obj):
                # print ("name ", name)
                if isinstance(obj, h5py.Dataset):
                    ...
                    truncated_length = truncated_slots*self.interact_motion_input_length*2
                    video_data = video_data[:truncated_length].reshape(truncated_slots, self.interact_motion_input_length*2, video_data.shape[-3], video_data.shape[-2], video_data.shape[-1])
            ...
            with h5py.File('/scratch/msy9an/data/tro_video/exo_1_{}.h5'.format(self.split), 'r') as file:
                ...
                dataset = file[all_interact_video_data[0][0]][all_interact_video_data[0][1]:all_interact_video_data[0][2]]
                dataset = file[all_interact_video_data[-1][0]][all_interact_video_data[-1][1]:all_interact_video_data[-1][2]]
                file.close()    
