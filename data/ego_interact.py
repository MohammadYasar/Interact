...
class INTERACTDataset(data.Dataset):
    ...
    def _load_summary(self, split_name):
        ...
        for i in range(len(unified_frames)):
            result_dict['{}/frames'.format(unified_frames[i,0])]= '{}'.format(unified_frames[i,8])
        print (result_dict)
        return result_dict
    ...
    def _load_all(self):
        ...
        with h5py.File('/scratch/msy9an/data/tro_video/skeleton_{}.h5'.format(self.split), 'r') as file:
            ...
            def print_hierarchy(name, obj):
                if isinstance(obj, h5py.Dataset):
                    ...
                    with h5py.File('/scratch/msy9an/data/tro_video/extracted_features_ego_2_{}.h5'.format(self.split), 'r') as video_file:
                        try:
        ...
        print (all_interact_motion_poses.shape)
        # all_interact_motion_poses = np.array(all_interact_motion_poses).reshape(-1, all_interact_motion_poses[0].shape[1])
    ...
    def _load_all_video_chunks(self):
        ...
        with h5py.File('/scratch/msy9an/data/tro_video/extracted_features_ego_2_{}.h5'.format(self.split), 'r') as file:
            # Print the keys (group names) at the top level
            # Iterate through the keys and print the hierarchy
            def print_hierarchy(name, obj):
                # print ("name ", name)
